...
TEST(client_get_order_history_returns_vector) {
    // This test verifies the get_order_history() method exists and returns a vector
    ...
    // Call get_order_history - should return empty vector if mock server not running
    // (won't crash, just returns empty on connection failure)
    std::vector<Order> order_history = client.get_order_history("2024-01-01");
...
int main(int argc, char* argv[]) {
    ...
    RUN_TEST(client_get_order_history_returns_vector);
